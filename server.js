...
app.get('/api/community/bootstrap',requireAuth,requirePremium,(req,res)=>{
  ...
  const statuses=readJson('statuses').filter(x=>new Date(x.expiresAt)>new Date()&&(x.userId===req.user.id||friendStatus(req.user.id,x.userId)==='accepted')).slice(-100).map(x=>({...x,user:publicUser(allUsers.find(u=>u.id===x.userId))}));
...
app.get('/api/community/status-media/:id',requireAuth,requirePremium,(req,res)=>{const s=readJson('statuses').find(x=>x.id===req.params.id);if(!s?.media)return res.sendStatus(404);if(s.userId!==req.user.id&&friendStatus(req.user.id,s.userId)!=='accepted')return res.sendStatus(403);res.sendFile(path.join(UPLOADS,s.media.path));});
...
app.patch('/api/profile',requireAuth,requireCsrf,(req,res)=>{const fields=['name','profession','institution','studyLevel','programme'];const users=readJson('users'),u=users.find(x=>x.id===req.user.id);if(u.role==='student'){if(req.body.studyLevel!==undefined&&!STUDY_LEVELS.includes(clean(req.body.studyLevel,80)))return res.status(400).json({error:'Select a valid study level from the list.'});if(req.body.programme!==undefined&&!PROGRAMMES.includes(clean(req.body.programme,120)))return res.status(400).json({error:'Select a valid programme from the list.'});}for(const f of fields)if(req.body[f]!==undefined)u[f]=clean(req.body[f],180);if(u.role==='student'&&!u.institution)return res.status(400).json({error:'Students need an institution selected.'});writeJson('users',users);res.json(safeUser(u));});
...
app.post('/api/auth/forgot-password',authLimiter,async(req,res)=>{
  ...
  else if(!mailed){ console.error(`Password reset email failed to send for ${u.email} (RESEND_API_KEY/RESEND_FROM_EMAIL missing or Resend request failed). The reset link was not exposed to the client.`); }
  res.json(response);
});
...
const NURSING_PROCESS_DIAGRAM=`<div class="lesson-diagram"><svg viewBox="0 0 600 170" width="100%" height="170" role="img" aria-label="The nursing process cycle: Assess, Diagnose, Plan, Implement, Evaluate">
<defs><marker id="arrowhead" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M0,0L10,5L0,10z" fill="#0c8d82"/></marker></defs>
<circle cx="60" cy="55" r="42" fill="#e9f4fb" stroke="#0c8d82" stroke-width="2"/><text x="60" y="59" text-anchor="middle" font-family="sans-serif" font-size="12" font-weight="700" fill="#0b3542">Assess</text>
<circle cx="185" cy="55" r="42" fill="#e9f4fb" stroke="#0c8d82" stroke-width="2"/><text x="185" y="59" text-anchor="middle" font-family="sans-serif" font-size="12" font-weight="700" fill="#0b3542">Diagnose</text>
<circle cx="310" cy="55" r="42" fill="#e9f4fb" stroke="#0c8d82" stroke-width="2"/><text x="310" y="59" text-anchor="middle" font-family="sans-serif" font-size="12" font-weight="700" fill="#0b3542">Plan</text>
<circle cx="435" cy="55" r="42" fill="#e9f4fb" stroke="#0c8d82" stroke-width="2"/><text x="435" y="59" text-anchor="middle" font-family="sans-serif" font-size="12" font-weight="700" fill="#0b3542">Implement</text>
<circle cx="560" cy="55" r="42" fill="#e9f4fb" stroke="#0c8d82" stroke-width="2"/><text x="560" y="59" text-anchor="middle" font-family="sans-serif" font-size="12" font-weight="700" fill="#0b3542">Evaluate</text>
<line x1="104" y1="55" x2="141" y2="55" stroke="#0c8d82" stroke-width="2" marker-end="url(#arrowhead)"/>
<line x1="229" y1="55" x2="266" y2="55" stroke="#0c8d82" stroke-width="2" marker-end="url(#arrowhead)"/>
<line x1="354" y1="55" x2="391" y2="55" stroke="#0c8d82" stroke-width="2" marker-end="url(#arrowhead)"/>
<line x1="479" y1="55" x2="516" y2="55" stroke="#0c8d82" stroke-width="2" marker-end="url(#arrowhead)"/>
<path d="M560,98 C560,150 60,150 60,98" fill="none" stroke="#8aa0a8" stroke-width="1.6" stroke-dasharray="5,4" marker-end="url(#arrowhead)"/>
<text x="310" y="166" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#61727b">Continuous cycle — re-assess after every intervention</text>
</svg></div>`;
function buildFallbackLessonNotes(module,lesson,lessonIndex,related){
  ...
  return `<h4>Learning focus — ${escapeHtml(angle.label)}</h4><p>This lesson, <b>${escapeHtml(lesson)}</b>, is where <b>${escapeHtml(module.title)}</b> gets specific about <b>${escapeHtml(coreTopic)}</b>. ${escapeHtml(angle.prompt)}</p>
<h4>Visual framework</h4><p>Use this as your reasoning map while you study <b>${escapeHtml(coreTopic)}</b> — most safe nursing and midwifery decisions follow this same cycle, from first contact to review.</p>${NURSING_PROCESS_DIAGRAM}
<h4>Core concepts to build, step by step</h4><ol><li><b>Define</b> ${escapeHtml(coreTopic)} in one or two plain sentences, without jargon.</li><li><b>Explain the mechanism or rationale</b> — why does this matter physiologically or professionally?</li><li><b>Recognise the key assessment findings</b> that tell you this concept is relevant right now.</li><li><b>Identify the priority action</b> — what should happen first, and why not something else?</li><li><b>Know when to escalate or refer</b> — which finding changes the plan and who needs to be told?</li></ol>
<p>Related areas in this module: ${siblingTopics.map(t=>`<b>${escapeHtml(t)}</b>`).join(', ')}. Compare each one against <b>${escapeHtml(coreTopic)}</b> and write down, in your own words, what is genuinely different.</p>
${worked}${secondQuestion}
<h4>Why it matters</h4><p>Strong nursing and midwifery practice depends on accurate assessment, safe communication, appropriate prioritisation, documentation, ethical practice and evidence-informed decisions. Ask yourself: <i>What would I assess first? Why? What finding would change my plan? Who should I escalate to?</i></p>
...
app.post('/api/ai/tutor',requireAuth,requirePremium,requireCsrf,async(req,res)=>{
  ...
  }catch(e){console.error('AI Tutor live call failed:',e.message);res.status(503).json({error:'The AI Tutor is temporarily unavailable right now. Please try again in a moment.',aiUnavailable:true});}
});
